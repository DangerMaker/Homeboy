...
public class BannerView extends RelativeLayout {
    ...
    private List<MovieModel> view_data;
...
//                        ? android.R.color.darker_gray : R.color.tab_host_text_sel_color));
            ...
            MovieModel item = view_data.get(position % view_data.size());
    ...
    public void setData(List<MovieModel> list) {
        if (list != null && list.size() > 0) {
            view_data = list;
            title.setText(view_data.get(0).name);
    ...
    private void addIndicator(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int item_width = ScreenUtil.dip2px(getContext(), 5);
...
//                    R.drawable.ic_banner_select : R.drawable.ic_banner_normal,null));
    ...
    private class BannerAdapter extends SimplePagerAdapter<MovieModel> {

        public BannerAdapter(List<MovieModel> list) {
            super(list);
        }
        ...
        public View getItemView(ViewGroup container, int position) {
            ...
            MovieModel item = view_data.get(position);
